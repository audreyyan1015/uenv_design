...
"application": [
("bridge_service.py","BridgeService","创建 run 时共用契约驱动的配置补值和校验函数；提交与收集统一批次"),
("sample_preparation.py","prepare_samples","互斥解析自带数据与 Hub 样本；原始行经 DatasetAdapter 转换，标准化行直接校验，统一输出 task/private_data"),
("request_builder.py","build_episode_request / build_batch_request","从公开 TaskSpec/RunSpec 构建请求和稳定 ID；公开 task 与私有 private_data 配对进入 EpisodeRequest，评分数据不进入 TaskSpec"),
...
("adapter_host.py","DatasetAdapterHost","受管理加载用户 DatasetAdapter，隔离其依赖")],
"interfaces": [
("client.py","UEnvClient","稳定 Python 用户入口"),
("cli.py","main","读取文件路径并调用 Bridge 共用配置函数；与 SDK 输入共用默认值、校验和提交，不提供同字段覆盖"),
...
("plan_resolver.rs","PlanResolver","转换请求与配置为唯一 ExecutionPlan；原位置锁定组件，tools 表只一份，runtime.image 只一处生效"),
...
("artifact_store.rs","ArtifactStore","轨迹产物存储及内容验证"),
...
("ports.rs","AgentHost / EnvironmentHost / ScorerHost / ToolHost / ModelProvider / ArtifactStore / ResultReporter / PackageCache","同一 ComponentHost 协议在 Worker 内按 Agent/Environment 权限拆成类型安全端口；ToolHost 由 ToolGateway 实现；另含评分、模型、产物、上报和包缓存边界"),
...
("agent_runtime.rs","AgentRuntime","模型、工具和环境动作的唯一受控入口；在副作用前检查预算与取消，再记录真实事件"),
("budget_enforcer.rs","BudgetEnforcer","权威截止时间、模型/工具/环境调用计数和评分预留"),
...
("scoring.rs","run_score","每个进入评分的 attempt 至多调用一次 Python Scorer，补全并校验 ScoreResult；不包含数据集规则"),
("trajectory_writer.rs","TrajectoryWriter","接收 Python 原始事件，分配顺序、校验身份、创建评分前快照并封存最终轨迹"),
...
("component_host_client.rs","ComponentHostClient","按 host 角色只暴露允许的 load/reset/step/run_agent/finalize/tool/close IPC"),
...
("ports.py","ModelProvider / RuntimeClient / EventClient","对 Rust 受控运行入口和模型适配的窄接口")],
"application":[
("component_host.py","ComponentHost","按启动角色加载 AgentRunner，或加载 Environment 与沙箱 Python 工具；不同权限使用不同进程实例"),
("scorer_host.py","ScorerHost","加载锁定 Scorer 并执行一次单条 episode 评分；不补全系统字段或封存轨迹"),
("tool_adapter.py","ToolAdapter","把 Rust Worker 已批准的工具表转换为 PlainAgent、MCP 或框架原生格式；不决定权限和预算"),
],
"infrastructure":[
("entrypoint_loader.py","EntryPointLoader","加载指定角色的版本化组件，校验接口和参数 schema；不管理模型循环或数据集业务"),
("model_client.py","ModelClient","规范模型请求/响应，保留真实 token 和版本"),
("runtime_client.py","RuntimeClient","调用 Rust Worker 的 exec/files/harness/tool/budget 能力"),
("event_client.py","EventClient","向 Rust TrajectoryWriter 提交原始事件；不分配最终序号或封存"),
...
"interfaces":[("component_host_rpc.py","ComponentHostRpc","接收角色限定的 load/reset/step/run/finalize/tool/close 命令；不接收完整 ExecutionPlan"),("scorer_host_rpc.py","ScorerHostRpc","接收一次单条 ScoreInput 和取消命令")],
...
"domain":[
("model.py","UEnvModel","数据集业务模型基类；作者使用 Python 类型标注，SDK 仅在边界自动封装 TypedConfig"),
("dataset.py","DatasetAdapter / PreparedSample","用户数据转换协议"),
("environment.py","Environment / Observation / Transition","环境动作、观测与冻结结果接口；与轨迹事件信封分开"),
("agent.py","AgentRunner","智能体开发接口；引用统一 Outcome"),
("outcome.py","Outcome","唯一执行结果类型，供 Agent、Environment 与 Scorer 引用；state 归 Environment 填写"),
("tool.py","ToolExecutor / ToolSpec","工具开发接口"),
("scoring.py","Scorer / ScoreInput / ScoreResult","Scorer.score 只负责单条 episode 评分；Server 只接纳一个权威结果，评测与训练复用")],
...
"crates/uenv-hub/src": {
"domain":[("package.rs","Package / ComponentVersion","不可变代码包发布身份"),("data_revision.rs","DataRevision / DataShard / SampleIndexEntry","不可变数据版本、分片和样本索引元数据"),("ports.rs","PackageRepository / DataRepository / ArtifactStore","代码包、数据索引和文件存储接口")],
...
"infrastructure":[("sqlite_repository.rs","SqliteRegistryRepository","保存代码包与数据 revision/索引元数据"),("artifact_store.rs","FileArtifactStore","按 digest 存储和下载代码、数据分片及产物")],
