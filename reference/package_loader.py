...
def expand_run(public_run: dict, manifest: dict, catalog: dict) -> dict:
    """Simulate CLI validation and creation of internal config envelopes."""
    if "schema_version" in public_run:
        raise ValueError("schema_version is generated by the submission boundary")
    run = deepcopy(public_run)
    ...
    for role in ("environment", "scorer"):
        ...
        run[role]["config"] = {
            "schema_ref": manifest["config_schemas"][role],
            "data": run[role].get("config", {}),
        }
    for role in ("agent", "backend"):
        ...
        run[role]["config"] = {"schema_ref": entry["config_schema"], "data": run[role].get("config", {})}
    for tool in run["tools"]:
        ...
        tool["config"] = {"schema_ref": entry["config_schema"], "data": tool.get("config", {})}
    return run
